// geeek website — sign-in gate.
// The website is static (no functions), so it talks to the APP's auth API
// cross-origin. The session cookie is scoped to .geeek.fm, so a sign-in on
// either property is shared with the other. Dormant until auth is configured.
(function () {
  "use strict";
  ...
  function check() {
    fetch(api("/session"), { credentials: "include", cache: "no-store" })
      ...
      .then(function (j) { if (!j || j.enabled === false || j.authed === true) hideGate(); })
      .catch(function () { hideGate(); }); // fail open to the site if the API is unreachable
  }
