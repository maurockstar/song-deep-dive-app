...
(function () {
  ...
  function enrichStoryMedia(payload) {
    ...
    curStoryKey = key;
    try {
      fetch(CFG.API_BASE + "/media?" + new URLSearchParams({ artist: t.artist || "", title: t.title || "", album: t.album || "", year: t.albumYear || "", v: "14" }).toString())
  ...
  async function loadMedia(track) {
    ...
    try {
      var res = await fetch(CFG.API_BASE + "/media?" + new URLSearchParams({ artist: track.artist || "", title: track.title || "", album: track.album || "", year: track.albumYear || "", v: "14" }).toString());
      if (!res.ok) throw 0;
