...
(function () {
  ...
  function enrichStoryMedia(payload) {
    ...
    curStoryKey = key;
    try {
      fetch(CFG.API_BASE + "/media?" + new URLSearchParams({ artist: t.artist || "", title: t.title || "", v: "2" }).toString())
  ...
  async function loadMedia(track) {
    ...
    try {
      var res = await fetch(CFG.API_BASE + "/media?" + new URLSearchParams({ artist: track.artist || "", title: track.title || "", v: "2" }).toString());
      if (!res.ok) throw 0;
