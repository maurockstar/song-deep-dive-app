...
async function writeDeeperWithClaude(apiKey, f, seed, similarPool, context) {
  ...
  const system =
    ...
    "- BE EXACT about the act's composition and the people: state the correct number of members (a duo is TWO people) and their roles, and remember the songwriter/producer is usually a MEMBER \u2014 never imply extra, uncredited people (e.g. do not say a duo's member wrote the song 'with the duo'). Factual precision about the artist, song, history and the people connected to them is non-negotiable.\n" +
...
function cleanRecos(arr, songArtist) {
  ...
  (Array.isArray(arr) ? arr : []).forEach(function (r) {
    ...
    if (r.why && BAD_WHY.test(String(r.why))) return; // drop language/region-only or off-genre picks
