...
async function smartCaptions(apiKey, ctx, arr, lang) {
  ...
  const esRule = (lang === "es")
    ? " WRITE THE CAPTIONS IN SPANISH: each title must be in natural, neutral Latin American Spanish (not from Spain, not machine-translated), still max 6 words REGISTER: write in neutral Chilean / standard Latin American Spanish. Use tu-form commands (toca, mira, busca, prueba, reproduce), NOT Argentine/Rioplatense voseo (no 'vos', no 'che', and never the accented voseo imperative forms). Prefer widely-neutral vocabulary (say 'tema' or 'cancion', not 'rola'). Keep it warm but neutral across Latin America., Mayúscula Inicial en las palabras principales. Keep venue names, band/artist and people's names in their ORIGINAL language (e.g. 'En vivo en Woodstock', 'En el estudio', 'Retrato de la banda'). Never add a year. Same rules otherwise."
    : "";
