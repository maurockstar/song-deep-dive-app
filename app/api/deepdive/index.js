...
const VERSION = "2.2"; // bump to invalidate the shared cache when the AI prompt changes
...
async function writeCardsWithClaude(apiKey, f, context, lang) {
  ...
  const system =
    ...
    "- BE EXACT about the act's composition and the people involved: state the correct number of members (a duo is TWO people, a trio THREE) and who they are and their roles. The songwriter/producer is very often a MEMBER of the act, so never phrase things to imply extra, uncredited people. For a duo, if one member wrote the song, say that member wrote it \u2014 do NOT say they wrote it 'with the duo' or 'with the band' (that invents a third person). When unsure of the lineup or who did what, stay general. This factual precision about the artist, the song, its history and the people connected to them is non-negotiable.\n" +
...
async function localizeCardsSpanish(apiKey, enPayload, context) {
  ...
  const body = { model: ANTHROPIC_MODEL, max_tokens: 1600, system: system, messages: [{ role: "user", content: user }] };
  const ctrl = new AbortController();
  ...
  try {
    ...
    if (!r.ok) { context.log("anthropic es cards http", r.status); return null; }
    const data = await r.json();
    const text = (data.content && data.content[0] && data.content[0].text) || "";
    const m = text.match(/\{[\s\S]*\}/);
    if (!m) return null;
    const parsed = JSON.parse(m[0]);
    if (!parsed || !parsed.story || !parsed.story.headline || !Array.isArray(parsed.story.body)) return null;
    return { story: parsed.story, cards: Array.isArray(parsed.cards) ? parsed.cards : (enPayload.cards || []) };
  } catch (e) { context.log("anthropic es cards error", e.message); return null; } finally { clearTimeout(timer); }
}
...
module.exports = async function (context, req) {
  ...
  // Phase 2 (or single-call) — SINGLE SOURCE OF TRUTH. The ENGLISH deep dive is generated + fact-checked
  // once (the canonical). Spanish is a faithful native re-authoring of that English, so the two languages
  // can never diverge on names, dates, credits or band composition (fixes cross-language factual drift).
  const ok = function (body) { context.res = { status: 200, headers: { "Content-Type": "application/json", "Cache-Control": "public, max-age=86400" }, body: body }; };
  async function buildEnglish() {
    let ai = null;
    if (apiKey) ai = await writeCardsWithClaude(apiKey, facts, context, "en");
    const used = !!(ai && ai.cards && ai.cards.length);
    const cards = used ? ai.cards : templateCards(facts);
    const story = (used && ai.story) ? ai.story : templateStory(facts, cards);
    return { payload: { track: { id: q.id, title: q.title, artist: q.artist }, cards: cards, story: story, _meta: { source: used ? "ai+open-data" : "open-data (add ANTHROPIC_API_KEY for AI)", year: facts.year || null, generatedAt: new Date().toISOString() } }, used: used };
  }
  capped(cache);

  if (lang === "es") {
    // Get/generate the ENGLISH canonical first (it fixes the facts), then render Spanish FROM it.
    const skeyEn = "sdd:" + VERSION + ":" + key;
    let en = cache.get(key);
    if (!(en && en.cards && en.cards.length)) en = await sharedGet(skeyEn);
    if (!(en && en.cards && en.cards.length)) {
      const b = await buildEnglish(); en = b.payload;
      if (b.used) { const won = await sharedSetNX(skeyEn, en); if (!won) { const ex = await sharedGet(skeyEn); if (ex && ex.cards && ex.cards.length) en = ex; } cache.set(key, en); }
    }
    let payload = en; // fall back to the English canonical if localization is unavailable
    if (apiKey && en && en.cards && en.cards.length && en.story) {
      const es = await localizeCardsSpanish(apiKey, en, context);
      if (es) {
        payload = { track: en.track, cards: es.cards, story: es.story, _meta: Object.assign({}, en._meta || {}, { lang: "es", localizedAt: new Date().toISOString() }) };
  ...
  // English canonical
  const b = await buildEnglish();
  let payload = b.payload;
  if (b.used) {
    const won = await sharedSetNX(skey, payload);
    if (!won) { const existing = await sharedGet(skey); if (existing && existing.cards && existing.cards.length) payload = existing; }
    cache.set(memKey, payload);
  } else if (!apiKey) {
    cache.set(memKey, payload);
  }
