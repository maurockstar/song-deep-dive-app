...
const MB_UA = "geeek/0.9.1 (https://geeek.fm)"; // MusicBrainz requires a UA
...
const VERSION = "0.9.1"; // bump to invalidate the shared cache when the AI prompt changes
...
async function writeCardsWithClaude(apiKey, f, context) {
  ...
  const user =
    `Facts:\n${factsBlock(f)}\n\n` +
    `Write five cards as STRICT JSON in exactly this shape. Keep every card VERY concise — bodies and extras must be SHORT, about half the length of a normal blurb. No filler. Titles tight, language vivid:\n` +
    `{"cards":[` +
    ...
    `]}`;
  const body = { model: ANTHROPIC_MODEL, max_tokens: 700, system, messages: [{ role: "user", content: user }] };
  const ctrl = new AbortController();
