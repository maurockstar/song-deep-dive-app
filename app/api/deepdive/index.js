...
const MB_UA = "SongDeepDive/0.6 (https://zealous-pond-0200e1e10.7.azurestaticapps.net)"; // MusicBrainz requires a UA
...
const VERSION = "0.6"; // bump to invalidate the shared cache when the AI prompt changes
...
async function writeCardsWithClaude(apiKey, f, context) {
  ...
  const user =
    `Facts:\n${factsBlock(f)}\n\n` +
    `Write five cards as STRICT JSON in exactly this shape (keep titles tight, bodies vivid):\n` +
    `{"cards":[` +
    ...
    `{"kicker":"Fun facts","title":"max 6 words","body":"1-2 quick, surprising fun facts about the song or the people who made it","extra":"2-3 more bite-sized facts or trivia — keep each one short and delightful"},` +
...
function templateCards(f) {
  ...
  return [
    ...
    { kicker: "Fun facts", title: "Quick facts", body: f.wikiArtist ? f.wikiArtist.slice(200, 360) : "Fun facts appear here once AI writing is on.", extra: f.wiki ? f.wiki.slice(560, 780) : "" },
